...
```sh
ELECTRON_RUN_AS_NODE=1 "$BUDDY_SERVICE_NODE" "$BUDDY_SERVICE_CLI" start worker -- sh worker.sh
```

这是短命令；实际服务由 Buddy 在独立进程中启动并登记。不要再使用裸 `nohup` 或手写脱离进程的脚本绕过登记。Buddy 不会自动接管历史上已经独立启动的进程。
